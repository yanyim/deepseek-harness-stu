...
| 05 | **问题版重讲**:20 行笨蛋 Agent 逐坑摔——九问九答(记账单位/插话/多策略/换配置不换内容/显示回放同源/事实存档过程不存/工具闸门/隔离/最后发言权),收官对照表;**Q3 为深挖版**(候选方案淘汰赛→cordis 瀑布 12 行实现→双向数据流→reject=blocked 源码实锤→设计回响;含瀑布机制修正与理解校准实录:只读共享+返回值向上替换、链是门卫≠step 循环) | guide/05 + qa/04 互链 |
