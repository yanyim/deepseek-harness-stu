...
| 05 | **问题版重讲**:20 行笨蛋 Agent 逐坑摔,九问九答 + 收官对照表;Q3 深挖版(候选淘汰赛→cordis 瀑布 12 行→双向数据流→reject=blocked 实锤→理解校准实录);**Q4-Q9 深改版**(Q4 直面「换适配器/流后钩子」两个直觉各错半格:换模型=改请求字段非换翻译官、流没有「后」;Q5 两份数据源三种漂移+投影思想;Q6 事实/算法本体论+回放=等价重建非复读+interrupted 补 closer;Q7 pre-execute 四姿态(deny 理由喂回/ask 升级审批)+post-execute 替换投影/block 闭环纠错+executionMode;Q8 工具泄漏=越权链+setup 两层理由;Q9 turn/end 六种死法表+merge-extensible) | guide/05 + qa/04 互链 |
